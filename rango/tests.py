...
class CategoryMethodTest(TestCase):
    def test_ensure_views_are_positive(self):
        """
        test_ensure_views_are_positive should result True for
        categories with views that are 0 or greater than 0
        """
...
def test_slug_line_creation(self):
    """
    test_slug_line_creation checks to make sure that when we add a
    category an appropriate slug line is created
    """
    cat = cat('Rando Category String')
    cat.save()
